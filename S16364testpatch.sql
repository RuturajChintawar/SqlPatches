...
-- DROP TABLE #clientWiseTrades  
 ...
 FROM (SELECT   
  ...
  SELECT   
   ...
   ClientPL,  
   DENSE_RANK() OVER (ORDER BY ClientTO DESC) AS RN ,
   COUNT(ClientTO) OVER (Partition by RefClientId)  CRN
 FROM #clientFinalData  
  WHERE ClientPL < 0
 SELECT  
  t.RefClientId,  
 ...
 Declare @Grp1PL INT,@Grp1TO INT,@Grp2PL INT,@Grp2TO INT  
...
--  update SysAmlReportSetting
--set Value='True'
--where RefAmlReportId=1237
--and Name='Active_In_Report'