--File:Tables:dbo:StagingRefBankMicr:CREATE
----WEB-84067-RC-START-----
GO
	CREATE TABLE dbo.StagingRefBankMicr
	(
		StagingRefBankMicrId INT IDENTITY(1,1) NOT NULL,
		BankId INT,
		[Name] VARCHAR(100),
		MicrNo VARCHAR(20),
		IfscCode VARCHAR(100),
		NeftCode VARCHAR(100),
		BankAddress1 VARCHAR(200),
		BankAddress2 VARCHAR(200),
		BankAddress3 VARCHAR(200),
		BankCity VARCHAR(100),
		BankState VARCHAR(100),
		BankCountry VARCHAR(100),
		BankZipCode INT,
		BankPhone1 VARCHAR(15),
		BankPhone2 VARCHAR(15),
		UniqueId INT,
		BankEmail VARCHAR(50),
		BankContactName VARCHAR(50),
		BankContactDesignation VARCHAR(50),
		...
		ALTER TABLE dbo.StagingRefBankMicr
		ADD CONSTRAINT [PK_StagingRefBankMicr] 
		PRIMARY KEY (StagingRefBankMicrId);
GO
----WEB-84067-RC-END-----

--File:Tables:dbo:RefBankMicr:ALTER
----WEB-84067-RC-START-----
GO
	ALTER TABLE dbo.RefBankMicr
	ADD BankId INT

	ALTER TABLE dbo.RefBankMicr
	ADD BankPhone1 VARCHAR(15)
	
	ALTER TABLE dbo.RefBankMicr
	ADD BankPhone2 VARCHAR(15)

	ALTER TABLE dbo.RefBankMicr
	ADD BankEmail VARCHAR(50)

	ALTER TABLE dbo.RefBankMicr
	ADD BankContactName VARCHAR(50)

	ALTER TABLE dbo.RefBankMicr
	ADD BankContactDesignation VARCHAR(50)
GO
----WEB-84067-RC-END-----

--File:StoredProcedures:dbo:RefBankMicr_InsertFromStagingRefBankMicr
----WEB-84067-RC-START-----
GO
	CREATE PROCEDURE dbo.RefBankMicr_InsertFromStagingRefBankMicr
	(
		@GUID VARCHAR(40)
	)
	AS 
	BEGIN
		...
		UPDATE stag
		SET stag.ErrorString = CASE WHEN ISNULL(stag.[Name],'') = '' AND ISNULL(stag.IfscCode,'') = ''THEN 'Ifsc code and Bank Name are mandatory fields'
		WHEN ISNULL(stag.[Name],'') = '' THEN 'Bank Name is mandatory field'
		WHEN ISNULL(stag.IfscCode,'') = '' THEN 'Ifsc Code is mandatory field'
		ELSE NULL END
		FROM dbo.StagingRefBankMicr stag
		WHERE stag.[GUID] = @InternalGuid 

		UPDATE stag
		SET stag.ErrorString = 'IFS Code length should be exact 11 digit alpha numeric value. Invalid value :'+stag.IfscCode COLLATE DATABASE_DEFAULT
		FROM dbo.StagingRefBankMicr stag
		WHERE stag.[GUID] = @InternalGuid AND LEN(ISNULL(stag.IfscCode,'')) <> 11 AND stag.ErrorString IS NULL

		UPDATE stag
		SET stag.ErrorString = 'Record with Ifsc Code '+stag.IfscCode COLLATE DATABASE_DEFAULT+' and Micr no '+stag.MicrNo COLLATE DATABASE_DEFAULT + 'already present in Bank Micr Master'
		FROM dbo.StagingRefBankMicr stag
		INNER JOIN dbo.RefBankMicr bank ON bank.MicrNo = stag.MicrNo AND bank.IfscCode = stag.IfscCode
		WHERE stag.[GUID] = @InternalGuid AND stag.ErrorString IS NULL

		UPDATE stag
		SET stag.ErrorString = 'Record with Ifsc Code '+stag.IfscCode COLLATE DATABASE_DEFAULT+ ' already present in Bank Micr Master'
		FROM dbo.StagingRefBankMicr stag
		INNER JOIN dbo.RefBankMicr bank ON bank.IfscCode = stag.IfscCode
		WHERE stag.[GUID] = @InternalGuid AND stag.ErrorString IS NULL

		UPDATE stag
		SET stag.ErrorString = 'Record with Micr no '+stag.MicrNo COLLATE DATABASE_DEFAULT+ ' already present in Bank Micr Master'
		FROM dbo.StagingRefBankMicr stag
		INNER JOIN dbo.RefBankMicr bank ON bank.MicrNo = stag.MicrNo
		WHERE stag.[GUID] = @InternalGuid AND stag.ErrorString IS NULL

		SELECT 
			stag.StagingRefBankMicrId,
			ROW_NUMBER() OVER(PARTITION BY stag.IfscCode , stag.MicrNo ORDER BY stag.AddedOn )rn
		INTO #tempDistinctIfscCode
		FROM dbo.StagingRefBankMicr stag
		WHERE stag.[GUID] = @InternalGuid

		SELECT
			stag.IfscCode
		INTO #duplicateIfscCode
		FROM dbo.StagingRefBankMicr stag
		WHERE stag.[GUID] = @InternalGuid 
		GROUP BY stag.IfscCode
		HAVING COUNT(stag.IfscCode) > 1

		SELECT
			stag.MicrNo
		INTO #duplicateMicrNo
		FROM dbo.StagingRefBankMicr stag
		WHERE stag.[GUID] = @InternalGuid 
		GROUP BY stag.MicrNo
		HAVING COUNT(stag.MicrNo) > 1

		UPDATE stag
		SET stag.ErrorString = 'Can not Insert Same Ifsc Code for different micr no in same file'
		FROM dbo.StagingRefBankMicr stag
		INNER JOIN #duplicateIfscCode dupCheck ON dupCheck.IfscCode = stag.IfscCode
		LEFT JOIN #duplicateMicrNo micrCheck ON micrCheck.MicrNo = stag.MicrNo
		WHERE micrCheck.MicrNo IS NULL AND stag.[GUID] = @InternalGuid AND stag.ErrorString IS NULL

		UPDATE stag
		SET stag.ErrorString = 'Can not Insert Same Micr no for different ifsc in same file'
		FROM dbo.StagingRefBankMicr stag
		INNER JOIN #duplicateMicrNo dupCheck ON dupCheck.MicrNo = stag.MicrNo
		LEFT JOIN #duplicateIfscCode IfscCheck ON IfscCheck.IfscCode = stag.IfscCode
		WHERE IfscCheck.IfscCode IS NULL AND stag.[GUID] = @InternalGuid AND stag.ErrorString IS NULL
		...
		SELECT
			...
			stag.IfscCode,
			stag.NeftCode,
			ISNULL(stag.BankAddress1 +', ','') + ISNULL(stag.BankAddress2 +', ','')+ISNULL(stag.BankAddress3 +', ','')+ ISNULL(stag.BankCity +', ','')+ISNULL(stag.BankState +', ','') + ISNULL(stag.BankCountry +', ','')+ISNULL(CONVERT(VARCHAR,stag.BankZipCode) +', ',''),
			CONVERT(VARCHAR,stag.UniqueId),
			stag.AddedBy,
			stag.AddedOn,
			stag.AddedBy,
			stag.AddedOn,
			...
			stag.BankPhone2,
			stag.BankEmail,
			stag.BankContactName,
			stag.BankContactDesignation
		FROM dbo.StagingRefBankMicr stag
		INNER JOIN #tempDistinctIfscCode dis ON dis.rn = 1 AND stag.StagingRefBankMicrId = dis.StagingRefBankMicrId
		WHERE stag.ErrorString IS NULL AND stag.[GUID] = @InternalGuid

		DECLARE @MinId INT
		SELECT @MinId = MIN(stag.StagingRefBankMicrId) FROM dbo.StagingRefBankMicr stag WHERE stag.[GUID] = @InternalGuid
		
		SELECT
			@ErrorString +CONVERT(VARCHAR , (stag.StagingRefBankMicrId - @MinId + 1))+ stag.ErrorString AS ErrorMessage
		FROM dbo.StagingRefBankMicr stag
		WHERE stag.ErrorString IS NOT NULL AND stag.[GUID] = @InternalGuid

		DELETE FROM dbo.StagingRefBankMicr WHERE [GUID] = @InternalGuid
	END
GO
----WEB-84067-RC-END-----

--File:StoredProcedures:dbo:AddOrUpdate_AddOrUpdateRefBankMicr
----WEB-84067-RC-START-----
GO
	CREATE PROCEDURE dbo.AddOrUpdate_AddOrUpdateRefBankMicr(
		...
		@BankId INT = NULL,
		@BankPhone1  VARCHAR(15) = NULL,
		@BankPhone2  VARCHAR(15) = NULL,
		@BankEmail  VARCHAR(50) = NULL,
		@BankContactName  VARCHAR(50) = NULL,
		@BankContactDesignation  VARCHAR(50) = NULL
		...
		BEGIN
			DECLARE @RefBankMicrIdInternal INT,@BankInternal VARCHAR(100),@MicrNoInternal VARCHAR(20),@IfscCodeInternal VARCHAR(100),
				...
				@UserNameInternal VARCHAR(50), @CurrDate DATETIME, @BankIdInternal INT, @BankPhone1Internal  VARCHAR(15), @BankPhone2Internal  VARCHAR(15), @BankEmailInternal  VARCHAR(50),
				@BankContactNameInternal  VARCHAR(50), @BankContactDesignationInternal  VARCHAR(50)
		 ...
		 SET @BankIdInternal = @BankId
		 SET @BankPhone1Internal = @BankPhone1
		 SET @BankPhone2Internal = @BankPhone2
		 SET @BankEmailInternal = @BankEmail
		 SET @BankContactNameInternal = @BankContactName
		 SET @BankContactDesignationInternal = @BankContactDesignation
		 ...
		 IF ISNULL(@RefBankMicrIdInternal,0) <> 0
			BEGIN
				...
				bank.BankId = @BankIdInternal,
				bank.BankPhone1 = @BankPhone1Internal,
				bank.BankPhone2 = @BankPhone2Internal,
				bank.BankEmail = @BankEmailInternal,
				bank.BankContactName = @BankContactNameInternal,
				bank.BankContactDesignation = @BankContactDesignationInternal
				...
				AND (ISNULL(bank.[Name],'') <> ISNULL(@BankInternal,'') OR
					...
					ISNULL(bank.OtherCode,'') <> ISNULL(@OtherCodeInternal,'') OR
					ISNULL(bank.BankId,'') <> ISNULL(@BankIdInternal,'') OR
					ISNULL(bank.BankPhone1,'') <> ISNULL(@BankPhone1Internal,'') OR
					ISNULL(bank.BankPhone2,'') <> ISNULL(@BankPhone2Internal,'') OR
					ISNULL(bank.BankEmail,'') <> ISNULL(@BankEmailInternal,'') OR
					ISNULL(bank.BankContactName,'') <> ISNULL(@BankContactNameInternal,'') OR
					ISNULL(bank.BankContactDesignation,'') <> ISNULL(@BankContactDesignationInternal,'') 
					)
			END
		 ELSE
			BEGIN
				...
				INSERT INTO dbo.RefBankMicr(
					...
					@BankPhone2Internal,
					@BankEmailInternal,
					@BankContactNameInternal,
					@BankContactDesignationInternal
				)
			END
		END
GO
...
--File:StoredProcedures:dbo:RefBankMICR_Search
----WEB-84067-RC-START-----
GO
	ALTER PROCEDURE [dbo].[RefBankMICR_Search]
(
...
AS
    ...
    SELECT bankMicr.RefBankMicrId,
		   bankMicr.Name,
		   bankMicr.MicrNo,
		   bankMicr.IfscCode,
		   bankMicr.NeftCode,
		   bankMicr.BranchDetails,
		   bankMicr.Address,
		   bankMicr.OtherCode,
		   bankMicr.AddedBy ,
           bankMicr.AddedOn ,
           bankMicr.EditedOn ,
           bankMicr.LastEditedBy ,
           ROW_NUMBER() OVER ( ORDER BY bankMicr.AddedOn DESC ) AS RowNumber,
		   bankMicr.BankId,
		   bankMicr.BankContactName,
		   bankMicr.BankContactDesignation
    INTO    #temp
    FROM dbo.RefBankMicr bankMicr
                
    ORDER BY bankMicr.EditedOn DESC
...
--File:Tables:dbo:StagingTssClientFormat:ALTER
----WEB-83671-RC-START-----
GO
...
--File:StoredProcedures:dbo:RefClient_InsertFromStagingTSSClientFormat
----WEB-83671-RC-START-----
GO
