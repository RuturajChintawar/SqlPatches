...
--WEB-63542-RC-START
GO
ALTER PROCEDURE [dbo].[CoreDpTransaction_InsertDpTransactionFromStaging]
(
	@Guid VARCHAR(40)
)
AS
...
BEGIN
    ...
    WHERE stage.[GUID] = @Guid AND stage.TransactionType IS NOT NULL AND NOT EXISTS 
    (
		SELECT 1 FROM dbo.RefDpTransactionType transtype
    ...
    SELECT @BadIsin = stage.ISIN FROM dbo.StagingDpTransaction stage WHERE
	 stage.[GUID] = @Guid
	AND NOT EXISTS
    (
    ...
    DECLARE @SegmentDatabaseId INT,@ISClientBoId INT,@ISClientCounterBoId INT
	SET @ISClientBoId=0
	SET @ISClientCounterBoId=0
	
	SELECT @ISClientBoId=1
	FROM dbo.StagingDpTransaction stage WHERE
	stage.[GUID] = @Guid AND 
	EXISTS(
	SELECT	1 
	...
	SELECT @ISClientCounterBoId=1
	FROM dbo.StagingDpTransaction stage WHERE
	stage.[GUID] = @Guid AND 
	EXISTS(
	SELECT	1 
	FROM	dbo.RefClient client 
	WHERE	stage.CounterBOId = client.ClientId )

    SELECT @SegmentDatabaseId = RefDatabaseId FROM RefSegmentEnum WHERE RefSegmentEnumId = @SegmentId
	DECLARE @BadClient VARCHAR(50)
    SELECT @BadClient = stage.BOId FROM dbo.StagingDpTransaction stage WHERE
	stage.[GUID] = @Guid
	AND
	(@ISClientBoId=0 OR @ISClientCounterBoId=0)
	...
	WHERE	[GUID] = @GUID
			AND @ISClientBoId=0
			AND @ISClientCounterBoId=0
			AND stage.TransactionStatus IN ('828','829','830','831','832','833','834','835','836','837','838')
			
    
    UPDATE dbo.StagingDpTransaction 
    SET DestatRejectionCode = NULL
    WHERE [GUID] = @GUID and TransactionType <> '32'
    
    UPDATE dbo.StagingDpTransaction 
    SET BuySellFlag = NULL
    WHERE [GUID] = @GUID and SegmentName <> 'NSDL' AND TransactionType NOT IN ('1','2','3','5')     
    
    UPDATE dbo.StagingDpTransaction 
    SET BuySellFlag = OrderStatusFlag
    WHERE [GUID] = @GUID and TransactionType  IN ('14','15','16','26','27','29')
	...
	SELECT		
		...
		stage.BusinessDate,		
		CASE WHEN transtype.CdslCode IN ('14' , '15' , '16' , '26'  , '27' , '29') THEN stage.OrderStatusFlag ELSE stage.BuySellFlag END,		
		CASE WHEN transtype.CdslCode IN ( '6' ,'7' , '8' , '9' , '10', '11', '32' , '33') THEN stage.FreeLockInFlag ELSE NULL END,
		CASE WHEN transtype.CdslCode = '12' THEN stage.FreezeLevel ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ( '17' , '18' , '28') THEN stage.TransactionTypeFlag ELSE NULL END,		
		...
		CASE WHEN transtype.CdslCode IN ('2' , '3' , '28') THEN stage.TradeId ELSE NULL END,
		...
		CASE WHEN transtype.CdslCode IN ('17' , '18') THEN stage.SenderTransactionReferenceNumber ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('20' , '21', '22') THEN stage.SequenceNumber ELSE NULL END,
		CASE WHEN transtype.CdslCode NOT IN ('6' , '7' , '8', '33') THEN stage.ExecutionDate ELSE NULL END,
		...
		CASE WHEN transtype.CdslCode NOT IN ('9' , '11', '12' , '18') THEN stage.RTAId ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('9' , '10') THEN stage.UnpledgePartCounter ELSE NULL END,
		...
		CASE WHEN transtype.CdslCode IN ('2' , '3' , '14' , '16' , '22' , '23' , '29') THEN stage.LockInId WHEN transtype.CdslCode = '18' THEN stage.DeliveryInstructionSlip WHEN transtype.CdslCode = '6' OR transtype.CdslCode = '7' OR transtype.CdslCode = '8' OR transtype.CdslCode = '9' OR transtype.CdslCode = '10' OR transtype.CdslCode = '11' OR transtype.CdslCode = '20' OR transtype.CdslCode = '21' OR transtype.CdslCode = '32' OR transtype.CdslCode = '33' THEN stage.EasiReferenceNumber ELSE NULL END,
		CASE WHEN transtype.CdslCode = '1' THEN stage.ParentTXNId ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('6' , '7' , '8' , '9' , '10' , '11' , '12' , '32') THEN stage.DocumentNumber ELSE NULL END,
		CASE WHEN transtype.CdslCode = '28' THEN stage.DIS ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('1' , '2' , '3', '4', '5') THEN stage.DeliveryInstructionSlip ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('6' , '32'  , '28') THEN stage.DispatchDocumentId ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('8' , '9' , '10' , '11') THEN stage.AgreementNumber ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('21' , '22') THEN stage.ParentLockInId ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('1' , '2' , '3' , '4' , '5') THEN stage.EasiAuthenticationRefNumber ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('6' , '32') THEN stage.CourierName ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('8' , '9' , '10' , '11') THEN stage.PledgeeDpInternalRefNumber ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('16' , '17' , '26' , '27' , '29') THEN stage.SelfBatchId ELSE NULL END,
		CASE WHEN transtype.CdslCode = '28' THEN stage.DespatchName ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('1' ,'8' , '9' ,'10','11') THEN stage.CASequenceNumber ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('6' , '33' , '28') THEN stage.NumberOfCertificates ELSE NULL END,
		...
		CASE WHEN transtype.CdslCode IN ('8' , '9' , '10' , '11') THEN stage.PledgeExpiryDate ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('12' , '28') THEN stage.FreezeExpiryDate ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('2' , '3') THEN stage.TransferType ELSE NULL END,
		CASE WHEN transtype.CdslCode = '7' THEN stage.ConfirmationRejectionFlag ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('17', '18') THEN stage.OrderStatusFlag WHEN transtype.CdslCode = '8' OR transtype.CdslCode = '9' OR transtype.CdslCode = '10' OR transtype.CdslCode = '11' OR transtype.CdslCode = '28' THEN stage.MakerCheckerFlag ELSE NULL END,
		...
		CASE WHEN transtype.CdslCode IN ('2' , '3' ,'28') THEN stage.OrderStatusFlag ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('1' , '30') THEN stage.PayInTypeFlag ELSE NULL END,
		CASE WHEN transtype.CdslCode = '6' THEN stage.DematRejectionFlag ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('7' , '33') THEN stage.LotType ELSE NULL END,
		...
		CASE WHEN transtype.CdslCode IN ('2' ,'3' , '28') THEN stage.ReasonForTrade ELSE NULL END,		
		CASE WHEN transtype.CdslCode = '6' THEN stage.DematRejectionCode ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('8' , '9' , '10' , '11') THEN stage.CAType WHEN transtype.CdslCode = '21' OR transtype.CdslCode = '22' THEN stage.TransactionCounter ELSE NULL END,
		...
		CASE WHEN transtype.CdslCode IN ('1', '4') THEN stage.EarmarkQuantity ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('6' , '28') THEN stage.VerifyAcceptedQuantity ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('7' , '33') THEN stage.AcceptedQuantity WHEN transtype.CdslCode = '32' THEN stage.ConfirmedAcceptedQuantity ELSE NULL END,
		CASE WHEN transtype.CdslCode = '8' THEN stage.PledgeValue ELSE NULL END,
		CASE WHEN transtype.CdslCode IN( '9' , '10' , '11')  THEN stage.TotalPledgedQuantity ELSE NULL END,
		CASE WHEN transtype.CdslCode = '12' THEN stage.SetupQuantity ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('15' ,'26') THEN stage.ActualQuantity ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('17' , '18') THEN stage.TransferredQuantity ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('6', '7' , '17' , '18', '32' , '33' ,'28') THEN stage.RejectedQuantity ELSE NULL END,
		CASE WHEN transtype.CdslCode = '8' THEN stage.PledgeBalace ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('9' , '10' , '11')  THEN stage.TotalQuantityUnpledgeConfiscation ELSE NULL END,
		CASE WHEN transtype.CdslCode IN( '6'  , '28') THEN stage.ConfirmedAcceptedQuantity ELSE NULL END,
		CASE WHEN transtype.CdslCode = '8' THEN stage.PledgeAvailableQuantity ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('9' , '10' , '11')  THEN stage.QuantityAvailableUnpledgeConfiscation ELSE NULL END,
		...
		CASE WHEN transtype.CdslCode IN ('6' , '28') THEN stage.ConfirmedRejectedQuantity ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('9' , '10' , '11')  THEN stage.TotalQtyUnpledgeConfiscationInitiated ELSE NULL END,
		CASE WHEN transtype.CdslCode = '32' THEN stage.TotalRejectedQuantity ELSE NULL END,
		stage.FinancialFlag,
		stage.EFTSCode,		
		transcode.RefDpTransactionCodeId,		
		CASE WHEN transtype.CdslCode NOT IN ('17' , '18' , '32', '21' , '22') AND stage.ModificationFlag = 1 THEN 1 WHEN transtype.CdslCode <> '17' AND transtype.CdslCode <> '18' AND transtype.CdslCode <> '32' AND transtype.CdslCode <> '21' AND transtype.CdslCode <> '22' AND stage.ModificationFlag = 0 THEN 0 ELSE NULL END,
		...
		CASE WHEN stage.SegmentName = 'NSDL' THEN stage.InternalRefNumber WHEN transtype.CdslCode = '7' THEN stage.DeliveryInstructionSlip WHEN transtype.CdslCode = '33' THEN stage.DocumentNumber WHEN transtype.CdslCode <> '33' AND transtype.CdslCode <> '8' AND transtype.CdslCode <> '9' AND transtype.CdslCode <> '10' AND transtype.CdslCode <> '11' THEN stage.InternalRefNumber ELSE NULL END,		
		CASE WHEN transtype.CdslCode IN ('7' , '33') THEN stage.RTAInternalRefNumber ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('8' , '9' , '10' , '11') THEN stage.PledgorDPInternalRefNumber ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('2' , '3' , '4')  THEN stage.TrustRemarks WHEN transtype.CdslCode = '1' THEN stage.RTARemarks ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('1' , '28') THEN stage.TxnRemarks ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('6', '32') THEN stage.DpRemarks ELSE NULL END,
		CASE WHEN transtype.CdslCode = '7' THEN stage.RejectionRemarks ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('8' , '9' , '10' , '11') THEN stage.PledgorDpRemarks ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('12' , '15' , '16' , '17' , '18' , '22' , '23' , '26' , '27' , '29' , '33') THEN stage.Remarks ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ( '6' , '7' , '32' , '33' )THEN stage.RTARemarks ELSE NULL END,
		CASE WHEN transtype.CdslCode = '28' THEN stage.CTRTransactionRemarks ELSE NULL END,
		CASE WHEN transtype.CdslCode IN ('8' , '9', '10' , '11') THEN stage.PledgeeDpRemarks ELSE NULL END,
		stage.OperatorId,		
		stage.TransactionSource,		
		stage.LastModificationDate,
		CASE WHEN transtype.CdslCode IN ('1','2', '3' , '4', '5' , '30') THEN stage.EasiReferenceNumber ELSE NULL END,
		...
		CASE WHEN transtype.CdslCode IN ('32' ,'33') THEN stage.OldNewDestatRequestNumber ELSE NULL END,
		...
		CASE WHEN transtype.CdslCode IN( '2', '3' , '5') then stage.DateOfIssue else null end,
		stage.Ucc,
		stage.SegmentCode,
		stage.ExchangeId,
		stage.TMId,
	...
	FROM 
		...
		dbo.RefDpTransactionType transtype ON (transtype.CdslCode = stage.TransactionType)LEFT JOIN		
		...
		transcode.RefDpTransactionStatusId IS NULL)) INNER JOIN		
		...
		dbo.RefIsin isin ON (isin.Name = stage.ISIN) 
...
GO
--WEB-63542-RC-END
------- complexity if was 4n earlier might be 2n now