--DEBUG -S843
GO
EXEC dbo.Sys_DropIfExists 'AML_GetDPTransactionOneToManyCDSLAndNSDL_S843_Custom','P'
GO
GO
CREATE PROCEDURE dbo.AML_GetDPTransactionOneToManyCDSLAndNSDL_S843_Custom (      
	 @RunDate DATETIME,      
...
BEGIN   
  DECLARE @RunDateInternal DATETIME, @ReportIdInternal INT,  @EndDate DATETIME , @StartDate DATETIME , @NsdlType904 INT, @NsdlType925 INT, @CdslType2 INT, @CdslType3 INT, @CdslType5 INT,       
     @CdslStatus305 INT, @CdslStatus511 INT, @CdslStatus521 INT ,@cdsl INT,@nsdl INT  ,@NoOfOOPClient INT, @NoOfInTxn INT, @TotalTxnValue DECIMAL(28,2) , @IsAlertDulicationAllowedInternal BIT  
   
  SELECT @ReportIdInternal = RefAmlReportId FROM dbo.RefAmlReport WHERE [Code] = 'S843'  
 ...
 CREATE TABLE #tradeData (        
   ...
   RefIsinId INT,       
   Quantity BIGINT,          
   CounterBOId VARCHAR(Max) COLLATE DATABASE_DEFAULT,  
 ...
 SELECT      
  ...
  DROP TABLE #selectedIsins  
	...
	SELECT
		...
		CASE
			WHEN ((t1.RefSegmentId = @cdsl AND t1.RefOppSegmentId = 0) 
				OR (t1.RefSegmentId = @nsdl AND t1.RefOppSegmentId = 1)) THEN @cdsl
			ELSE @nsdl END AS OppSegmentId
	INTO #finaldata
	FROM #tradeData t1
  ...
  (  
   ...
   AND alerts.UniqueTradeReferenceNo =  fd.OppSegmentId
  )  
...
GO
GO
EXEC dbo.AML_GetDPTransactionOneToManyCDSLAndNSDL_S843_Custom
	@RunDate = '2022-04-04', -- rundate (yyyy-MM-dd),
    @NoOfOOPClientInternal = 1, -- No of Opp Client Id =>
   @NoOfInTxnInternal = 1, --No of Out transactions during the month =>
   @TotalTxnValueInternal   =  1 -- Total Out Transactions Value during the month = >
...
BEGIN     
     ...
     @CdslStatus305 INT, @CdslStatus511 INT ,@cdsl INT,@nsdl INT  ,@NoOfOOPClient INT, @NoOfInTxn INT, @TotalTxnValue DECIMAL(28,2) , @IsAlertDulicationAllowedInternal BIT, @CdslStatus521 INT
