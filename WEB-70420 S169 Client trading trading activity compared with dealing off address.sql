...
SELECT @ExchangeRefEnumvalueId = dbo.GetEnumValueId('AmlReportType','AmlReportType1') --exchange
...
INSERT INTO dbo.RefAmlReport (
	...
	1264,
	'S169 Client Trading Activity Compared With Dealing Off Address',
	'System',
	GETDATE(),
	'System',
	GETDATE(),
	...
	@RefAlertRegisterCaseTypeId,
	0, 
	'S169ClientTradingActivityComparedWithDealingOffAddress',
	1,
	@FrequencyFlagRefEnumValueId,
	169,
	...
	<b>Thresholds:</b> <br>
	1. Client TO: It is the sum of Individual Turnover of the client in all scrips in the one same segment in given lookback period.  It will generate alerts if the Client Turnover is greater than or equal to the set threshold. <br>
	2. No. of Traded Unique PIN: These are the unique PIN codes that the client has traded on the run date in one particular segment. It will work on greater than or equal to basis.  It will generate alerts If the these traded PIN are totally different from the PIN mentioned in the client KYC. (Correspondence, Permanent address pin of the client and Pins of all the intermediary as per intermediary master ). The Top ''X'' PIN''s will be shown in alert output as per the threshold. <br>
	3. Lookback Period: This is the Lookback days that system will check the trades in past ''X'' days for alert generation. Lookback period is configurable but maximum of 30 days.<br> 
	4. Exclude PIN: It is a Flat manual textbox threshold. User can add the PIN Codes which they want to exclude from alert generation. The PIN''s are to be entered comma seperated. ( e.g. 400101, 396235 ) <br>
	<b>Note:</b><br>
	1. Alerts will be generated & TO will be calculated segment wise. <br>
	2. The number of Traded Unique PIN should be totally different from the Client PIN''s to generate alert. <br>',
	@ExchangeRefEnumvalueId
)
GO
--WEB-70420-END-RC
--WEB-70420-START-RC
...
SELECT @RefAmlReportId = RefAmlReportId FROM dbo.RefAmlReport WHERE [Name] = 'S169 Client Trading Activity Compared With Dealing Off Address'
...
INSERT INTO dbo.RefProcess (
	...
	'S169 Client Trading Activity Compared With Dealing Off Address',
	'TSS.SmallOfficeWeb.ManageData.Processes.AML.Reports.S169ClientTradingActivityComparedWithDealingOffAddress',
...
GO
--WEB-70420-END-RC
EXEC AML_GetClientTradingActivityComparedWithDealingOffAddress '03-11-2022',1264
--WEB-70420-START-RC
GO
...
BEGIN 
	 ...
	 SELECT 
			@ClientToThreshold = CONVERT(DECIMAL(28,2),[Value]) 
	 FROM dbo.SysAmlReportSetting 
	 ...
	 CREATE TABLE #clientsToExclude
	(
		RefClientId INT
	)

	INSERT INTO #clientsToExclude
	SELECT DISTINCT
		RefClientId
	...
	SELECT 
		...
		COALESCE(SUM(CONVERT(DECIMAL(28,2), ROUND(trade.Rate * trade.Quantity,2))),0) AS turnover
	INTO #tradeDetailsWitPin
	FROM dbo.CoreTrade trade
	INNER JOIN  dbo.RefClient ref ON trade.RefSegmentId IN ( @BSECashId, @NSECashId , @NSEFNOId , @NSECDXId) AND ( trade.TradeDate BETWEEN @LookBackDate AND @ToDate ) AND ref.RefClientId = trade.RefClientId  
	...
	LEFT JOIN #ExcludePin pins ON pins.pin = SUBSTRING (CONVERT(VARCHAR(MAX),trade.CtclId), 1,6)
	WHERE 
		SUBSTRING (CONVERT(VARCHAR(MAX),trade.CtclId), 1,6) NOT IN ('111111','333333', ISNULL(ref.CAddressPin,''), ISNULL(ref.PAddressPin,''), ISNULL(inter.Pin,'')) AND
		ex.RefClientId IS NULL AND
		pins.pin IS NULL
	GROUP BY
		trade.RefClientId,
		trade.RefSegmentId,
		SUBSTRING (CONVERT(VARCHAR(MAX),trade.CtclId), 1,6)
	...
	SELECT 
		...
		STUFF(( SELECT ', '+ pinforstuff.Pin      
			...
			pinforstuff.RefSegmentId = trade.RefSegmentId
	...
	GROUP BY
	 ...
	 SELECT
		...
		trade.RefSegmentId,
		seg.Segment,
		ISNULL(ref.CAddressPin ,'')+', '+ISNULL(ref.PAddressPin,'') AS ClientPIN,
	 ...
	 WHERE trade.uniquepin > = @UniquePinThreshold AND trade.totalto > = @ClientToThreshold
