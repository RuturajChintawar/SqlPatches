...
GO
	...
	INSERT INTO dbo.RefAmlReport (
		...
		1301,
		'S198 Client Purchase to income in a specified period',
		'System',
		GETDATE(),
		'System',
		GETDATE(),
		...
		@RefAlertRegisterCaseTypeId,
		1, 
		'S198ClientPurchaseToIncomeInASpecificPeriod',
		1,
		@FrequencyFlagRefEnumValueId,
		198,
		'Client TO Net Buy',
		'This scenario will help to identify the whether Client''s net buy turnover is commensurating with the Income Strength<br>
		Segments:BSE Cash , NSE Cash ; Frequency: Daily ; Period: Lookback period<br>
		Thresholds: <br>
		<b>1. Client TO Net Buy :</b> Net Buy value of the client in a specified lookback period with respect to buy and sell value<br>
		<b>2. Income Multplier ::</b> No of times of income, upto which Client TO Net buy value will be considered inline.  If the Client TO Net buy value breaches more than ''X'' no of times of income, an alert will be generated.<br>
		<b>3. Lookback Period (Days) ::</b> Period for which Client TO net buy will be calculated<br>
		<b>4. Client Type ::</b> Category of the client<br>'
	)
GO
--WEB-84978-END RC
...
GO
	...
	SELECT @AmlReportId = RefAmlReportId FROM dbo.RefAmlReport WHERE [Name] = 'S198 Client Purchase to income in a specified period'
...
GO
	...
	SELECT @RefAmlReportId = RefAmlReportId FROM dbo.RefAmlReport WHERE [Name] = 'S198 Client Purchase to income in a specified period'
	...
	INSERT INTO dbo.RefProcess (
		...
		'S198 Client Purchase to income in a specified period',
		'TSS.SmallOfficeWeb.ManageData.Processes.AML.Reports.S198ClientPurchaseToIncomeInASpecificPeriod',
...
--File:StoredProcedures:dbo:AML_GetClientPurchaseToIncomeInASpecifiedPeriod
--WEB-84978-START RC
GO
	CREATE PROCEDURE dbo.AML_GetClientPurchaseToIncomeInASpecifiedPeriod 
		(     
			@TradeDate DATETIME,      
			@RefAmlReportId INT
		)      
	...
	BEGIN 
		...
		DECLARE @RefAmlReportIdInternal INT, @TradeDateInternal DATETIME, @FromDate DATETIME,  @BSECashId INT, @NSECashId INT,
			@LookBack INT, @IncomeMultiplier DECIMAL(28,6), @ProfileDefault INT, @DefaultIncome DECIMAL(28, 2), 
			@DefaultIncomeAbove1Cr DECIMAL(28, 2), @ClientPurchaseToIncomeId INT
		...
		SET @IncomeMultiplier = (SELECT CONVERT(DECIMAL(28,6),syst.[Value]) FROM dbo.SysAmlReportSetting syst 
							WHERE syst.RefAmlReportId = @RefAmlReportIdInternal AND syst.[Name] = 'Default_Income')
		...
		SELECT
			...
			(t.BSECashBuyValue + t.NSECashBuyValue) - (t.BSECashSellValue + t.NSECashSellValue) AS NetBuy
		INTO #clientWiseData
		FROM(	SELECT
					RefClientId,
		...
		INNER JOIN dbo.RefClient cl ON tr.NetBuy > 0  AND cl.RefClientId = tr.RefClientId 
		...
		SELECT 
			...
			@IncomeMultiplier AS IncomeMultiplier,
			(inc.Income * @IncomeMultiplier) AS IncomeStrength,
			pti.BSECashBuyValue,
		...
		INNER JOIN #scenarioRules rules ON inc.RefClientStatusId = rules.RefClientStatusId AND pti.NetBuy >= rules.Threshold
		WHERE pti.NetBuy >= (inc.Income * @IncomeMultiplier) 
...
GO
	...
	SUM(CASE WHEN report.Code = 'S849' THEN 1 ELSE 0 END) S849,
	SUM(CASE WHEN report.Code = 'S851' THEN 1 ELSE 0 END) S851,
	SUM(CASE WHEN report.Code = 'S852' THEN 1 ELSE 0 END) S852,
	SUM(CASE WHEN report.Code = 'S853' THEN 1 ELSE 0 END) S853,
	SUM(CASE WHEN report.Code = 'S854' THEN 1 ELSE 0 END) S854,
	SUM(CASE WHEN report.Code = 'S855' THEN 1 ELSE 0 END) S855,
	SUM(CASE WHEN report.Code = 'S856' THEN 1 ELSE 0 END) S856,
	SUM(CASE WHEN report.Code = 'S857' THEN 1 ELSE 0 END) S857,
	SUM(CASE WHEN report.Code = 'S858' THEN 1 ELSE 0 END) S858,
	SUM(CASE WHEN report.Code = 'S859' THEN 1 ELSE 0 END) S859,
	SUM(CASE WHEN report.Code = 'S850' THEN 1 ELSE 0 END) S850,
	SUM(CASE WHEN report.Code = 'S194' THEN 1 ELSE 0 END) S194,
	SUM(CASE WHEN report.Code = 'S193' THEN 1 ELSE 0 END) S193,
	SUM(CASE WHEN report.Code = 'S196' THEN 1 ELSE 0 END) S196,
	SUM(CASE WHEN report.Code = 'S860' THEN 1 ELSE 0 END) S860,
	SUM(CASE WHEN report.Code = 'S861' THEN 1 ELSE 0 END) S861,
	SUM(CASE WHEN report.Code = 'S862' THEN 1 ELSE 0 END) S862,
	SUM(CASE WHEN report.Code = 'S197' THEN 1 ELSE 0 END) S197,
	SUM(CASE WHEN report.Code = 'S198' THEN 1 ELSE 0 END) S198,
	SUM(CASE WHEN report.Code = 'S199' THEN 1 ELSE 0 END) S199
	 INTO #Temp          
	...
	S849 = CASE WHEN t.S849 = 0 THEN NULL ELSE t.S849 END,
	S851 = CASE WHEN t.S851 = 0 THEN NULL ELSE t.S851 END,
	S852 = CASE WHEN t.S852 = 0 THEN NULL ELSE t.S852 END,
	S853 = CASE WHEN t.S853 = 0 THEN NULL ELSE t.S853 END,
	S854 = CASE WHEN t.S854 = 0 THEN NULL ELSE t.S854 END,
	S855 = CASE WHEN t.S855 = 0 THEN NULL ELSE t.S855 END,
	S856 = CASE WHEN t.S856 = 0 THEN NULL ELSE t.S856 END,
	S857 = CASE WHEN t.S857 = 0 THEN NULL ELSE t.S857 END,
	S858 = CASE WHEN t.S858 = 0 THEN NULL ELSE t.S858 END,
	S859 = CASE WHEN t.S859 = 0 THEN NULL ELSE t.S859 END,
	S850 = CASE WHEN t.S850 = 0 THEN NULL ELSE t.S850 END,
	S194 = CASE WHEN t.S194 = 0 THEN NULL ELSE t.S194 END,
	S193 = CASE WHEN t.S193 = 0 THEN NULL ELSE t.S193 END,
	S196 = CASE WHEN t.S196 = 0 THEN NULL ELSE t.S196 END,
	S860 = CASE WHEN t.S860 = 0 THEN NULL ELSE t.S860 END,
	S861 = CASE WHEN t.S861 = 0 THEN NULL ELSE t.S861 END,
	S862 = CASE WHEN t.S862 = 0 THEN NULL ELSE t.S862 END,
	S197 = CASE WHEN t.S197 = 0 THEN NULL ELSE t.S197 END,
	S198 = CASE WHEN t.S198 = 0 THEN NULL ELSE t.S198 END,
	S199 = CASE WHEN t.S199 = 0 THEN NULL ELSE t.S199 END
	 FROM #Result r      
	...
	SUM(CASE WHEN report.Code = 'S849' THEN 0 ELSE NULL END) AS S849,
	SUM(CASE WHEN report.Code = 'S851' THEN 0 ELSE NULL END) AS S851,
	SUM(CASE WHEN report.Code = 'S852' THEN 0 ELSE NULL END) AS S852,
	SUM(CASE WHEN report.Code = 'S853' THEN 0 ELSE NULL END) AS S853,
	SUM(CASE WHEN report.Code = 'S854' THEN 0 ELSE NULL END) AS S854,
	SUM(CASE WHEN report.Code = 'S855' THEN 0 ELSE NULL END) AS S855,
	SUM(CASE WHEN report.Code = 'S856' THEN 0 ELSE NULL END) AS S856,
	SUM(CASE WHEN report.Code = 'S857' THEN 0 ELSE NULL END) AS S857,
	SUM(CASE WHEN report.Code = 'S858' THEN 0 ELSE NULL END) AS S858,
	SUM(CASE WHEN report.Code = 'S859' THEN 0 ELSE NULL END) AS S859,
	SUM(CASE WHEN report.Code = 'S850' THEN 0 ELSE NULL END) AS S850,
	SUM(CASE WHEN report.Code = 'S194' THEN 0 ELSE NULL END) AS S194,
	SUM(CASE WHEN report.Code = 'S193' THEN 0 ELSE NULL END) AS S193,
	SUM(CASE WHEN report.Code = 'S196' THEN 0 ELSE NULL END) AS S196,
	SUM(CASE WHEN report.Code = 'S860' THEN 0 ELSE NULL END) AS S860,
	SUM(CASE WHEN report.Code = 'S861' THEN 0 ELSE NULL END) AS S861,
	SUM(CASE WHEN report.Code = 'S862' THEN 0 ELSE NULL END) AS S862,
	SUM(CASE WHEN report.Code = 'S197' THEN 0 ELSE NULL END) AS S197,
	SUM(CASE WHEN report.Code = 'S198' THEN 0 ELSE NULL END) AS S198,
	SUM(CASE WHEN report.Code = 'S199' THEN 0 ELSE NULL END) AS S199
	INTO    #Runs          
	...
	S849 = CASE WHEN result.S849 IS NULL THEN run.S849 ELSE result.S849 END,
	S851 = CASE WHEN result.S851 IS NULL THEN run.S851 ELSE result.S851 END,
	S852 = CASE WHEN result.S852 IS NULL THEN run.S852 ELSE result.S852 END,
	S853 = CASE WHEN result.S853 IS NULL THEN run.S853 ELSE result.S853 END,
	S854 = CASE WHEN result.S854 IS NULL THEN run.S854 ELSE result.S854 END,
	S855 = CASE WHEN result.S855 IS NULL THEN run.S855 ELSE result.S855 END,
	S856 = CASE WHEN result.S856 IS NULL THEN run.S856 ELSE result.S856 END,
	S857 = CASE WHEN result.S857 IS NULL THEN run.S857 ELSE result.S857 END,
	S858 = CASE WHEN result.S858 IS NULL THEN run.S858 ELSE result.S858 END,
	S859 = CASE WHEN result.S859 IS NULL THEN run.S859 ELSE result.S859 END,
	S850 = CASE WHEN result.S850 IS NULL THEN run.S850 ELSE result.S850 END,
	S194 = CASE WHEN result.S194 IS NULL THEN run.S194 ELSE result.S194 END,
	S193 = CASE WHEN result.S193 IS NULL THEN run.S193 ELSE result.S193 END,
	S196 = CASE WHEN result.S196 IS NULL THEN run.S196 ELSE result.S196 END,
	S860 = CASE WHEN result.S860 IS NULL THEN run.S860 ELSE result.S860 END,
	S861 = CASE WHEN result.S861 IS NULL THEN run.S861 ELSE result.S861 END,
	S862 = CASE WHEN result.S862 IS NULL THEN run.S862 ELSE result.S862 END,
	S197 = CASE WHEN result.S197 IS NULL THEN run.S197 ELSE result.S197 END,
	S198 = CASE WHEN result.S198 IS NULL THEN run.S198 ELSE result.S198 END,
	S199 = CASE WHEN result.S199 IS NULL THEN run.S199 ELSE result.S199 END
