GO
	...
	AS
	BEGIN
		DECLARE @AlertIdInternal BIGINT, @RefClientId BIGINT, @RunDate DATETIME, @ToDate DATETIME, @AuditIdEmailChange INT, @AuditIdBankAccNoChangeState INT,
			@AuditIdMobileChangeState INT, @AuditIdPanChangeState INT, @AuditIdPAddressChangeState INT, @AuditIdCAddressChangeState INT, @RefEntityTypeId INT

		SET @AlertIdInternal = @AlertId
		SELECT @RefClientId = alert.RefClientId, @RunDate = alert.ReportDate, @ToDate = CONVERT(DATETIME, DATEDIFF(dd, 0, alert.ReportDate)) + CONVERT(DATETIME, '23:59:59.000') 
		...
		SELECT
			DENSE_RANK() OVER(PARTITION BY 1 ORDER BY aud.AuditDateTime DESC) AuditId,
			aud.Email,
			aud.Mobile,
			aud.PAN,
			ISNULL(aud.PAddressLine1 + ', ', '') + ISNULL(aud.PAddressLine2 + ', ', '') +     
			ISNULL(aud.PAddressLine3 + ', ', '') + ISNULL(aud.PAddressPin + ', ', '') +     
			ISNULL(aud.PAddressCity + ', ', '') + ISNULL(aud.PAddressState + ', ', '') +     
			ISNULL(aud.PAddressCountry + ', ', '') AS PAddress,
			ISNULL(aud.CAddressLine1, '') + ISNULL(aud.CAddressLine2, '') +     
			ISNULL(aud.CAddressLine3, '') + ISNULL(aud.CAddressPin, '') +     
			ISNULL(aud.CAddressCity, '') + ISNULL(aud.CAddressState, '') +     
			ISNULL(aud.CAddressCountry, '') AS CAddress,
		...
		FROM dbo.RefClient_Audit aud WHERE aud.RefClientId = @RefClientId AND aud.AuditDmlAction = 'Update'
			AND aud.AuditDateTime BETWEEN @RunDate AND @ToDate
		...
		INNER JOIN #tempCilentChanges aud2 ON aud1.AuditId = aud2.AuditId AND aud1.AuditState + aud2.AuditState = 1
			AND( ISNULL(aud1.Email, '') <> ISNULL(aud2.Email, '') OR  ISNULL(aud1.Mobile, '') <> ISNULL(aud2.Mobile, '') 
			OR ISNULL(aud1.PAN, '') <> ISNULL(aud2.PAN, '') OR dbo.RemoveMatchingCharacters(aud1.PAddress, '^0-9a-z') <> dbo.RemoveMatchingCharacters(aud2.PAddress, '^0-9a-z')
			OR dbo.RemoveMatchingCharacters(aud1.CAddress, '^0-9a-z') <> dbo.RemoveMatchingCharacters(aud2.CAddress, '^0-9a-z')) 
		...
		SELECT
			DENSE_RANK() OVER(PARTITION BY t.AuditDateTime) AuditId,
			t.BankAccNo,
			t.AuditState
		INTO #tempBankChanges
		FROM (
				SELECT
					bank.AuditDateTime,
					bank.BankAccNo,
				...
				WHERE bank.BankAccNo <> '' AND bank.RefClientId = @RefClientId AND bank.AuditDmlAction = 'Update' AND bank.AuditDateTime BETWEEN @RunDate AND @ToDate
				...
				UNION
				  ...
				  WHERE bank.BankAccountNo <> '' AND bank.EntityId = @RefClientId AND bank.RefEntityTypeId = @RefEntityTypeId  AND bank.AuditDmlAction = 'Update' AND bank.AuditDateTime BETWEEN @RunDate AND @ToDate
			)t
